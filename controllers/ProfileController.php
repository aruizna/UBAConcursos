...
class ProfileController extends Controller
{
    ...
    public function actionAgregarcargo()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $cargo->profile_id = $_POST['profile_id'];
        ...
        if ($this->request->isPost) {
            try {
                if (!empty($this->request->post("Profile")["id"])) {
                    $dataProvider = Profile::findOne(["id" => $this->request->post("Profile")["id"], "concurso_id" => (array_key_exists("concurso_id", $this->request->post("Profile")) ? $this->request->post("Profile")["concurso_id"] : null)]);
                }
                if (empty($dataProvider)) {
                    $dataProvider = new Profile();
                }
                if ($dataProvider->load($this->request->post())) {
                    $prevProfile = Profile::findOne(['user_id' => Yii::$app->user->id, 'concurso_id' => (!empty($dataProvider->concurso_id) ?  $dataProvider->concurso_id : null)]);
                    if (!empty($prevProfile) && $prevProfile->id != $dataProvider->id)  $dataProvider->id = $prevProfile->id; # If it is first Time , Profile to concurso is not set, so  Create one
    ...
    public function preinscripcion($id)
    {
        ...
        $preinscripto->concurso_id = $profile->concurso_id;
    ...
    public function previsualizar($id)
    {
        ...
        $profile = Profile::findOne(['id' => $id]);
        $concurso = Concurso::findOne(['id_concurso' =>  $profile->concurso_id]);
        ...
        try {
            ...
            $pdf->Output('attachments/formularios/Recibo_Preinscripcion-' . $profile->cuil . '_' . $profile->concurso_id . '-' . $facultad->nombre_facultad . '-' . date('Ymd_His') . '.pdf', 'F');
            return true;
        } catch (Exception $e) {
            Error($e->getMessage());
            return false;
